...
Комментарий по крайним случаям: `415`, `429` и `503` я вынес отдельно как расширенные технические сценарии. Для минимального описания регистрации обычно хватило бы `201`, `400`, `409` и `500`, но эти случаи добавлены осознанно: они закрывают неверный `Content-Type`, возможный перебор регистраций и зависимость от внешнего сервиса reCAPTCHA.
...
10. Проверяет лимит попыток регистрации.
    - Например, по IP-адресу или `userName`.
    - Если лимит превышен, вернуть `429 Too Many Requests`.

11. Отправляет токен на проверку в сервис reCAPTCHA.
...
flowchart TD
    ...
    R -- Да --> L{Лимит попыток не превышен?}
    L -- Нет --> E429[429 Too Many Requests]

    L -- Да --> RV[Проверить reCAPTCHA на сервере]
    RV --> RVQ{reCAPTCHA пройдена?}
    RVQ -- Нет --> E400CAP

    RVQ -- Да --> U{userName уже существует?}
...
OpenAPI-спецификация приложена отдельным файлом: `задача 3-openapi-register.yaml`.
